...
class ET_Builder_Module_Fullwidth_Slider extends ET_Builder_Module {
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		$output = sprintf(
			'<div%4$s class="royalSlider rsDefault">
				<div class="et_pb_slides">
					%2$s
				</div> <!-- .et_pb_slides -->
			</div> <!-- .et_pb_slider -->
		...
		if( $projects->post_count > 0 ) {
			while ( $projects->have_posts() ) {
				...
				<?php
					...
					list($thumb_src, $thumb_width, $thumb_height) = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), array('full') );

					$orientation = ( $thumb_height > $thumb_width ) ? 'portrait' : 'landscape';


					$src_thumb = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), 'full' );

					
					if ( '' !== $thumb_src ) : ?>
						<div class="et_pb_portfolio_image <?php echo esc_attr( $orientation ); ?>">
							<img src="<?php echo $src_thumb[0]; ?>" alt="<?php echo esc_attr( get_the_title() ); ?>"/>
							<div class="meta">
								<a href="<?php esc_url( the_permalink() ); ?>">
								<?php
									$data_icon = '' !== $hover_icon
